...
function ArrowFromBuoy({ position, direction, speed, isLive }) {
...
function getStatusLabel(status) {
  const labels = {
    ONLINE: { text: 'Live', color: 'bg-green-100 text-green-700' },
    STALE: { text: 'Data Lama', color: 'bg-yellow-100 text-yellow-700' },
    OFFLINE: { text: 'Offline', color: 'bg-red-100 text-red-700' },
    NEVER_SEEN: { text: 'Belum Aktif', color: 'bg-gray-100 text-gray-600' },
  };
  return labels[status] || labels.NEVER_SEEN;
}

export function Location() {
  const [data, setData] = useState(null);
  const [buoyStatus, setBuoyStatus] = useState({
    buoy1: 'NEVER_SEEN', buoy2: 'NEVER_SEEN', buoy3: 'NEVER_SEEN'
  });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // Fetch latest dari API saat load
  ...
  if (loading) {
    return <div className="p-6 max-w-7xl mx-auto"><div className="text-center py-12 text-gray-500">Memuat peta...</div></div>;
  }
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className={`mb-4 px-4 py-2 rounded-lg border flex items-center justify-between ${
        ...
        <div className="flex items-center gap-2">
          {isSystemActive ? <Wifi className="w-4 h-4 text-green-600" /> : <WifiOff className="w-4 h-4 text-yellow-600" />}
          <span className={`text-sm font-medium ${isSystemActive ? "text-green-700" : "text-yellow-700"}`}>
            {isSystemActive ? `${onlineCount}/3 Buoy Online` : "Hardware Offline - Menampilkan data terakhir"}
          </span>
        </div>
        {data && data.prediction && (
          <span className={`text-xs px-3 py-1 rounded-full font-bold ${
            data.prediction === 'Danger' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'
          }`}>
      ...
      <Card className="mb-4 shadow-sm">
        ...
        <CardContent className="text-xs text-gray-600 space-y-1">
          <p>• Marker <span className="text-green-700 font-semibold">hijau</span> = Online + Aman, <span className="text-red-700 font-semibold">merah</span> = Online + Bahaya</p>
          <p>• Marker <span className="text-yellow-700 font-semibold">kuning</span> = Data Lama (buoy belum update &gt; 10 detik)</p>
          <p>• Marker <span className="text-gray-600 font-semibold">abu-abu</span> = Offline atau belum pernah aktif</p>
          <p>• Garis utuh = data live, garis putus-putus = data lama</p>
        </CardContent>
      </Card>
      ...
      <Card className="shadow-sm overflow-hidden">
        <CardContent className="p-0" style={{ height: '600px' }}>
          <MapContainer center={mapCenter} zoom={17} style={{ height: '100%', width: '100%' }} scrollWheelZoom={true}>
            <TileLayer
            ...
            {data && [1, 2, 3].map(nodeId => {
              ...
              const status = buoyStatus[`buoy${nodeId}`] || 'NEVER_SEEN';
              const prediction = data.prediction || 'Safe';
              const isLive = status === 'ONLINE';
              
              if (!lat || !lon || lat === 0 || lon === 0) return null;
              
              const buoyLabel = nodeId === 1 ? 'Buoy 1 (Kiri)' : nodeId === 2 ? 'Buoy 2 (Tengah)' : 'Buoy 3 (Kanan)';
              const statusLabel = getStatusLabel(status);
              
              return (
                <div key={nodeId}>
                  <Marker position={[lat, lon]} icon={createBuoyIcon(status, prediction, nodeId)}>
                    <Popup>
                      <div className="text-sm" style={{ minWidth: '180px' }}>
                        <p className="font-bold mb-2 text-base">{buoyLabel}</p>
                        <span className={`inline-block px-2 py-0.5 rounded text-xs font-semibold ${statusLabel.color} mb-2`}>
                          {statusLabel.text}
                        </span>
                        <div className="space-y-1">
                          ...
                          <p>Gelombang: <span className="font-semibold">{wave?.toFixed(2)}</span></p>
                          <p className="text-xs mt-2 text-gray-500 border-t pt-1">{lat.toFixed(6)}, {lon.toFixed(6)}</p>
                        </div>
                      </div>
                    </Popup>
                  </Marker>
                  <ArrowFromBuoy position={[lat, lon]} direction={dir} speed={speed} isLive={isLive} />
                </div>
              );
            })}
          </MapContainer>
              ...
              <Card key={nodeId} className="shadow-sm">
                <CardContent className="p-3 text-xs space-y-1">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded ${statusLabel.color}`}>{statusLabel.text}</span>
                  </div>
                  <p>v={speed?.toFixed(2)} m/s, arah={Math.round(dir)}°, wave={wave?.toFixed(2)}</p>
                  {hasGps ? (
                    <p className="text-gray-500 text-xs">{lat.toFixed(4)}, {lon.toFixed(4)}</p>
                  ) : (
      ...
      {!data && (
        <Card className="mt-4 shadow-sm">
          <CardContent className="p-6 text-center text-gray-500">
            ...
            <p className="text-xs mt-2">Nyalakan minimal 1 buoy untuk menampilkan posisi.</p>
          </CardContent>
        </Card>
      )}
    </div>
